Sadrzaj
* [Uvod](#uvod)
* [Osnovni statisticki pokazatelji](#osnovni-statisticki-pokazatelji)
  * [Neprekidna obelezja](#neprekidna-obelezja)
    * [Trimovana sredina](#trimovana-sredina)
* [Grafici](#grafici)
  * [Boxplot](#boxplot)
  * [Barplot](#barplot)
  * [Scatter](#scatter)
  * [Pie](#pie)
* [Tabele kontingencije](#tabele-kontingencije)
* [Testiranje normalnosti](#testiranje-normalnosti)
...
Pre merenja su promenljive, a nakon merenja imamo fiksne vrednosti. Tada se obelezavaju malim slovom.
...
**Varijacioni niz** je niz sortiranih (asc) slucajnih promenljivih na osnovu njihovih realizacija.

Clanovi varijacionog niza se nazivaju statistikama poretka.
...
CTMS tvrdi da prethodni zakljucak nije posledica slucajno (ili tendenciozno) odabranog uzorka, vec da ce 
...
Prvo radimo razliku izmedju date vrednosti i sredine kako bi odredili udaljenost. Nakon toga kvadriramo kako znak 
...
**Kategorijska obelezja** dele uzorak na kategorije, na primer **pol** (2 kategorije: muski i zenski) ili 
...
**Neprekidna obelezja** su sva ona obelezja koja nisu kategorijska.
...
**Tretiraju se isto kao i kategorijska.**

Sta karakterise kategorijska obelezja, kao sto je, na primer, za pol?
...
- skewness i kurtosis, koji opisuju odstupanje od normalne raspodele (ne moraju se koristiti za 
...
porediti sa obicnom sredinom radi provere uticaja outlier-a.
...
**Kvantili** su presecne tacke koje dele opseg distribucije verovatnoce na intervale sa jednakim verovatnocama. **Kvartil** je kvantil koji deli opseg na 4 intervala sa po 25%.
...
U suprotnom se, ako su dovoljno bliske, ne radi se nista.
...
Zove se jos i **stubicasti dijagram**. 

Koristi se za kategorijska obelezja.

Prikazuje nam ucestalost, tj. broj ispitanika u grupi. To je glavni podatak koji se dobija iz ovoga grafika.
...
Mozemo i dodato legendu sa `legend.text=T`.
...
Tumacenje izgleda: 
* **zavisnost**: gusto rasporedjene tacke
* **nezavisnost**: retko rasporedjene tacke

Sto su tacke gusce rasporedjene, to je zavisnost veca.

Zavisnost je **pozitivna** ako sa povecanjem jednog dolazi i do povecanja drugog, a **negativna** ako sa 
...
Tabele kontingencije nam daju brojno stanje u tih `x * y` grupa. 

Koristimo funkciju `ctable` iz paketa `summarytools`. 

Table kontingencije sa procentom po vrstama (100% dobijamo sumiranjem po vrsti):
```R
ctable(x, y)
```
...
Ako ne zelimo da prikazemo procenat, koristimo `prop="n"`.

# Testiranje normalnosti

* [Razlika izmedju p-value i alpha](https://www.statology.org/p-value-vs-alpha/)
